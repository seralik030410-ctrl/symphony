...
The optional `scripts/check_site.py` performs an offline structural check. Read it first, then run it only through `skill.run_script`; never bypass the approval shown by FinCtrl.
