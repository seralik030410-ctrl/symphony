...
$output = Join-Path $releaseRoot 'FinCtrl-Windows.zip'
...
git -C $projectRoot archive --format=zip --output=$output --prefix=FinCtrl/ HEAD
if ($LASTEXITCODE -ne 0) { throw 'git archive failed.' }
$hash = (Get-FileHash -Algorithm SHA256 -LiteralPath $output).Hash
Set-Content -LiteralPath ($output + '.sha256.txt') -Value "$hash  FinCtrl-Windows.zip" -Encoding ascii
