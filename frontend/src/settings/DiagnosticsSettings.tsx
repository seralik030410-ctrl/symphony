...
export function DiagnosticsSettings() {
  ...
  return <><h2>Диагностика</h2><section className="settings-card diagnostics-card">
    {error ? <p role="alert"><WarningCircle size={17} /> {error}</p> : report ? <>
      <div className="settings-row"><div><strong>{report.platform} · {report.architecture}</strong><small>FinCtrl {report.release} · Python {report.python} · SQLite {report.sqlite}</small></div><span>{report.checks.every(item => item.ready) ? <CheckCircle size={17} /> : <WarningCircle size={17} />} {report.checks.filter(item => item.ready).length}/{report.checks.length} готово</span></div>
