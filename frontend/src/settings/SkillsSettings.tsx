...
function SkillInstallDialog({ onClose, onInstalled }: { onClose: () => void; onInstalled: (id: string) => void }) {
  ...
  return <Dialog title="Установить навык" onClose={() => { if (!busy) onClose(); }}><header className="dialog-toolbar"><div><h2>Установить навык</h2><small>Импортируется в управляемую локальную папку FinCtrl</small></div><button className="icon-button" aria-label="Закрыть установку" onClick={onClose}><X size={18} /></button></header>
    ...
    {kind === "zip" ? <><label className="install-field"><span>ZIP-файл (до 10 МБ)</span><input type="file" accept=".zip,application/zip" onChange={event => setFile(event.target.files?.[0] ?? null)} /></label><label className="install-field"><span>Подкаталог навыка (если в ZIP их несколько)</span><input value={source} onChange={event => setSource(event.target.value)} placeholder="skills/my-skill" /></label></>
      : <label className="install-field"><span>{kind === "folder" ? "Полный путь к локальной папке" : "HTTPS Git или GitHub URL (без submodules)"}</span><input value={source} onChange={event => setSource(event.target.value)} placeholder={kind === "folder" ? "C:\\Users\\…\\my-skill" : "https://github.com/owner/repo/tree/main/path/to/skill"} /></label>}
