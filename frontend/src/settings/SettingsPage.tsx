...
export function SettingsPage({ session, profiles, active, onPolicy, onModel, onClose, onSessionSaved }: {
  ...
  return <main className="settings-page">
    <aside className="settings-nav"><button className="settings-back" onClick={onClose}><ArrowLeft size={18} /> Вернуться в приложение</button><strong>Настройки</strong><nav aria-label="Разделы настроек"><button aria-current={section === "general" ? "page" : undefined} onClick={() => setSection("general")}><Gear size={17} /> Общее</button><button aria-current={section === "context" ? "page" : undefined} onClick={() => setSection("context")}><Brain size={17} /> Контекст и память</button><button aria-current={section === "research" ? "page" : undefined} onClick={() => setSection("research")}><Globe size={17} /> Интернет</button><button aria-current={section === "skills" ? "page" : undefined} onClick={() => setSection("skills")}><Package size={17} /> Навыки</button></nav><small>FinCtrl · Этап 7 в работе</small></aside>
