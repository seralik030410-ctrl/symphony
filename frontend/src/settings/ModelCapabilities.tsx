...
export function ModelCapabilities({ session, active, onSaved }: { session: Session; active: boolean; onSaved: () => void }) {
  ...
  return <details className="context-budget-footer">
    ...
    <div className="settings-card">
      ...
      <label className="settings-row"><span>Предел контекста провайдера, токенов</span><input type="number" min={1024} max={1048576} value={maximum} disabled={active || busy || loading} onChange={event => setMaximum(Number(event.target.value))} /></label>
    </div>
    <p>Настройка не добавляет модели новые способности. Для текстовой модели используйте OCR: изображение останется локальным.</p>
    <button className="settings-primary" disabled={active || busy || loading || !Number.isInteger(maximum) || maximum < 1024 || maximum > 1048576} onClick={() => void save()}>Сохранить возможности</button>
