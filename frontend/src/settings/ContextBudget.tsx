...
export function ContextBudget({ session, active, onSaved }: {
  ...
  const validOutput = Number.isInteger(outputCount) && outputCount >= 64 && outputCount <= 65536 && outputCount < windowSize;
  ...
  return <section className="context-budget" aria-labelledby="context-budget-heading">
    ...
    <div className="settings-card">
      <div className="settings-row"><div><strong>Длина контекста</strong><small>Общий бюджет истории, памяти, источников, инструментов и ответа. Сохраняется отдельно для этого чата.</small></div>
        <CustomSelect ariaLabel="Длина контекста" value={String(windowSize)} disabled={active || saving || maximum === null} onChange={value => { setWindowSize(Number(value)); setNotice(""); }} options={values.map(value => ({ value: String(value), label: `${value / 1024}K · ${value.toLocaleString("ru-RU")} токенов`, description: value === 16384 ? "Быстрый стандарт" : value === 32768 ? "Больше истории и источников" : value >= 65536 ? "Выше расход памяти и время обработки" : "Компактное окно", unavailable: maximum !== null && value > maximum }))} />
      </div>
      <div className="settings-row"><div><label htmlFor="context-output-limit"><strong>Максимальная длина ответа</strong></label><small>Резерв внутри выбранного окна; провайдер может завершить ответ раньше.</small></div>
        <input id="context-output-limit" type="number" min={64} max={Math.min(65536, windowSize - 1)} step={1} value={output} disabled={active || saving} aria-invalid={!validOutput} onChange={event => { setOutput(event.target.value); setNotice(""); }} />
      </div>
      <div className="context-budget-footer">
        ...
        {windowSize >= 65536 ? <p>64K потребует больше RAM/VRAM и времени на обработку истории.</p> : null}
        ...
        {!validOutput ? <p role="alert">Лимит ответа — целое число от 64 до 65 536 и меньше окна контекста.</p> : null}
